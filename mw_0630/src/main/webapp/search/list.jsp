...
<%
  ...
  // case 4 : inner="where title like '%"+search+"%' "+"|| content like '%"+search+"%' "+ "|| name like '%"+search+"%'"; break;
   }
...
<script>
  ...
  <input type="submit" value="검색">
    ...
    <% 
     ...
     %> 
    <tr>
      <td> <a href="readnum.jsp?id=<%=rs.getString("id")%>&search=<%=search%>&field=<%=field%>&pager=<%=pager%>"> <%=rs.getString("title")%> </a> </td>
    ...
    <!-- 이전 10페이지 이동(이전 그룹) : 첫그룹에서는 링크 X -->
      ...
      <%
        ...
        {
      %> 
       <a href="list.jsp?pager=<%=pstart-1%>&search=<%=search%>&field=<%=field%>"> << </a>
      <%
        }
      %>
    <!-- 이전 1페이지 이동 : 현재페이지가 1페이지이면 링크 X -->
      ...
      <%
       ...
       {
      %> 
       <a href="list.jsp?pager=<%=pager-1%>&search=<%=search%>&field=<%=field%>"> < </a>
    <% 
       }
    ...
    <!-- 다음 10페이지 이동 : 마지막 그룹이라면 링크 X-->
      ...
      <%
       ...
       {
      %> 
       <a href="list.jsp?pager=<%=pend+1%>&search=<%=search%>&field=<%=field%>"> >> </a>
      <%
       }
      %> 
      </td>
