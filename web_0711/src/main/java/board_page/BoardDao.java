...
public class BoardDao {
	...
	public void write_ok(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		// 쿼리 생성
		String sql="insert into board2(title,name,content,pwd,writeday) values(?,?,?,?,now())";
	...
	public ResultSet list(HttpServletRequest request) throws Exception
	{
		...
		int page=getnow(request);  // 현재페이지 구하는 메소드 호출
		// 시작인덱스 구하기
		int index=(page-1)*10;
	...
	{
		int page=getnow(request);  // 현재페이지를 구해주는 페이지 메소드 호출
    ...
    public int getnow(HttpServletRequest request) // request를 이용하여 
    {                                             // 현재페이지 구하기
    	int page;
    ...
    {
    	String sql="select ceil(count(*)/10) as chong from board2";
    	pstmt=conn.prepareStatement(sql);
    	ResultSet rs=pstmt.executeQuery();
    	rs.next();
	...
	public void update_ok(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		{
		  // 쿼리 생성
		  String sql="update board2 set title=?, name=?, content=? where id=?";
	...
	public void bimil_check(HttpServletRequest request,HttpServletResponse response) throws Exception
	{
		...
		// 쿼리 생성
		String sql="select count(*) as cnt from board2 where pwd=? and id=?";
