...
<script>
		...
		function delmemo(id,name,content)
		{
			document.pkc.action="delete.jsp";
			document.pkc.id.value=id;
			document.pkc.submit.value="글삭제";
			document.pkc.content.value=content;
			document.pkc.name.value=name;
		}
	</script>
...
<body>	<!-- 0615/memo/memo.jsp -->
	...
	<td onclick="delmemo(<%=rs.getInt("id")%>,'<%=rs.getString("name") %>','<%=rs.getString("content") %>')" ><%=rs.getString("writeday") %></td>
	</tr>
	<%
		}
	%>
