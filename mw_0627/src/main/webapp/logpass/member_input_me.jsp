...
function pwd_check(my){ // 두번째 비밀번호를 입력할때마다 첫번째와 두번째를 비교하여 같으면 같다, 틀리면 틀리다
	
	var pwd = my.pwd.value;
	var pwd2 = my.pwd2.value;
	
	if(pwd == pwd2){
		document.getElementById("aa").innerText="비밀번호가 맞습니다"
...
function userid_check(){ // 아이디를 서버로 보내어 체크
	...
	//location="userid_check_old.jsp?userid="+userid; => userid_check_old.jsp
	
	var uid = document.pkc.userid.value;
	...
	chk.onreadystatechange = function(){
		if(chk.readyState == 4){ // 호출한 jsp가 모든 동작을 완료
			...
			// num은 0 아니면 1입니다.
			if(num == 0){
				document.getElementById("uc").innerText="사용 가능한 아이디입니다"
				document.getElementById("uc").style.color="blue";
				ck=2;
			}
			else{
				document.getElementById("uc").innerText="사용 불가능한 아이디입니다"
		...
		document.getElementById("uc").innerText="빈칸입니다"
		document.getElementById("uc").style.color="red";
	}
	else if (uid.length < 6){
		document.getElementById("uc").innerText="6자리 미만입니다";
		document.getElementById("uc").style.color="red";
	}
}
var ck=1;
...
<body>	<!-- member_input.jsp => 회원정보 입력폼 -->
	...
	<form method="post" name="pkc" action="member_input_ok.jsp" onsubmit="return check()">
	<table width="500" align="center">
	<caption><h2>회원 가입</h2></caption>
		<tr>
		<td>아이디</td>
		<td>
		<input type="text" name="userid" onkeyup="userid_eng_check(this)" placeholder="6자리이상" size="10"> <!-- onkeyup="userid_eng_check(this)" -->
		...
		<tr>
		<td>비밀번호확인</td>
		<td><input type="password" name="pwd2" onkeyup="pwd_check(this.form)" size="6"><span id="aa"></span></td>
		</tr>
		<tr>
		<td>전화번호</td>
		...
		<tr>
		<td>주소</td>
		<td><input type="text" name="juso1" size="20"></td>
		</tr>
		<tr>
		<td>상세주소</td>
