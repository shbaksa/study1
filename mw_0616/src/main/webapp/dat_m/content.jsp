...
<body>
	...
	rs.next();
	String datpwd = rs.getString("pwd");
	rs.close();
	rs = pstmt.executeQuery();
	%>
	...
	<td onclick="dat_view(<%=rs.getInt("id")%>,<%=rs.getInt("bid") %>,1)" > <%=rs.getString("name") %> </td>
	<td colspan="2"><%=rs.getString("content") %></td>	
	<td onclick="dat_view(<%=rs.getInt("id")%>,<%=rs.getInt("bid") %>,2)"> <%=rs.getString("writeday") %> </td> <!-- 날짜를 클릭하면 삭제폼이 보이게 -->
	</tr>
	<% } %>
	</tbody>
	<tfoot>
	...
	<form name="pkc1" id="dat" method="post"  align="center" >
	<input type="text" name="id">
	<input type="text" name="bid" value="<%=id %>">
	<input type="password" name="pwd" placeholder="비밀번호" size="5">
	<input type="submit" name="submit"  >
	</form>
	<%-- <form name="pkc" id="dat" method="post"  align="center" >
	<input type="hidden" name="id">
	<input type="hidden" name="bid" value="<%=id %>">
	<input type="password" name="pwd" placeholder="비밀번호" size="5">
	<input type="submit" name="cst" val >
	</form> --%>
	...
	<script>
		...
		function dat_view(id,bid,n)  {
			...
			if(n==2) {
				...
				let url ="dat_pwd.jsp?id=";
				let url_1 ="&bid=";
				open(url+id+url_1+bid,"","width=600,height=100");
