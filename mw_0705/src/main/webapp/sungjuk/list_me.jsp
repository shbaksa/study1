...
<body>
	...
	while(rs.next()){ %>
	<tr>
	<td><input type="checkbox" name="delcheck" value="check<%=rs.getInt("id") %>"></td>
	...
	<td>
	<input type="button" id="bu<%=rs.getInt("id") %>" class="chbu" onclick="view(<%=i%>,<%=rs.getInt("id")%>,'<%=rs.getString("name") %>',<%=rs.getInt("kor")%>,<%=rs.getInt("eng") %>,<%=rs.getInt("mat") %>)" value="수정"></td>
	<td><input type="button" onclick="del_move(<%=rs.getInt("id") %>)" value="삭제"></td>
	</tr>
	<% i++; } %>
	
	<tr>
	<td colspan="9" align="center"><input type="button" onclick="del()" value="성적입력">
	</td>
