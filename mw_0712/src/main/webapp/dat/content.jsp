  ...
  <table width="500" border="1" align="center">
    ...
    <%
       /*  String area;
        ...
        int num = rs.getInt("area");
        String[] temp = {"서울","경기","강원","충청","전라"};
        String area = temp[num];
    %>
    <tr>
      <td> 지 역 </td>
      <td> <%=area%> </td>
  ...
  <table width="500" border="0" align="center">
    <tr>
     <form name="form" method="post" action="dat_write_ok.jsp">
      ...
      <input type="hidden" name="bid" value="<%=rs.getInt("id")%>"> <!-- 댓글을 달 board2 글의 id -->
   ...
   <%
    // dat테이블의 내용을 가져오기
    // 쿼리 생성
    ResultSet rs2 = bd.dat_list(rs.getInt("id")); // rs 는  board 테이블의 id
    
    // 댓글테이블의 list는 rs2에 저장
    while(rs2.next())  // 댓글 리스트 출력
    {
   %> <!-- 이름을 클릭하면 수정메뉴,  날짜를 클릭하면 삭제로 이동 -->
    <tr> <!-- dat테이블의 내용을 출력 -->
      <td onclick="dat_update(<%=rs2.getInt("id")%>,'<%=rs2.getString("name")%>','<%=rs2.getString("content")%>')"> <%=rs2.getString("name")%> </td>
      <td colspan="2"> <%=rs2.getString("content")%> </td>
      <td onclick="dat_view(<%=rs2.getInt("id")%>)"> <%=rs2.getString("writeday")%> </td> <!-- 날짜를 클릭하면 삭제폼이 보이게 -->
    </tr>    <!-- 숨겨진 삭제폼을 보이게 -->
   <%
    }
   %> 
...
3. submit의 value를 댓글수정으로 변경 -->
   ...
   <form name="pkc" id="dat_del" method="post" action="dat_delete.jsp">
     ...
     <!-- board2테이블의 id추가 -->
     <input type="hidden" name="bid" value="<%=rs.getInt("id")%>">
